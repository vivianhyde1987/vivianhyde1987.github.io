...
function setupCabinExperience() {
  ...
  const plantRoomSources = [...new Set(getBlogMaterials().library?.plants || [])];
  const plantRoomImages = plantRoomSources.map((src, index) => {
    const plant = document.createElement("img");
    plant.className = `cabin-plant-room-item cabin-plant-room-item--${index + 1}`;
    plant.alt = "植物房里的植物";
    plant.loading = "lazy";
    plant.decoding = "async";
    plant.dataset.plantRoomSrc = src;
    plant.hidden = true;
    scene.append(plant);
    return plant;
  });
  ...
  const cabinAssetLayers = [
    ...
    ...plantRoomImages.map((plant, index) => [`plantRoom${index + 1}`, plant])
  ];
  ...
  const rooms = {
    ...
    plants: { number: "ROOM 06", title: "白墙植物房", note: "白墙、柔光和所有植物安静地站在一起。", image: "", alt: "白墙植物房", aspect: "landscape", detail: "", detailAlt: "", prop: "plants", pendant: false, assets: { title: "白墙植物房" }, layout: { theme: "plants", paintingSize: "0%", paintingHeight: "0%", paintingX: "0%", paintingY: "0%", propSize: "0%", propX: "0%", propY: "0%", plantSize: "0%", plantX: "0%", plantY: "0%", accentSize: "0%", accentX: "0%", accentY: "0%", pendantSize: "0%", moodColor: "#e8dfd0", floorTone: "#eee7dc" } }
  };
  let lightOn = false;
  ...
  const applyCabinRoomAssets = (roomId, room = rooms[roomId]) => {
    ...
    const isPlantRoom = roomId === "plants";
    if (isPlantRoom) {
      image.hidden = true;
      detailImage.hidden = true;
      plantImage.hidden = true;
      accentImage.hidden = true;
      pendantImage.hidden = true;
      flowerExtraLeft.hidden = true;
      flowerExtraRight.hidden = true;
      plantRoomImages.forEach((plant, index) => {
        const source = plant.dataset.plantRoomSrc;
        plant.hidden = !source;
        if (source) setImageSafe(plant, source, `植物房里的植物 ${index + 1}`);
      });
      image.alt = room.alt;
    ...
    image.hidden = false;
    plantRoomImages.forEach((plant) => { plant.hidden = true; });
...
function setupMimiPetV2() {
  ...
  const floorPlanes = {
    ...
    room6: { top: 0.70, bottom: 0.91, left: 0.07, right: 0.76, spots: [[0.12, 0.84], [0.31, 0.84], [0.56, 0.82]] }
  };
